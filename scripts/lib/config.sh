...
load_monan_jedi_config() {
  ...
  # read_config.py emits only shell-quoted exports after strict validation.
  ...
  [[ -n "${PROJECT_ROOT:-}" ]] || {
    log_error "PROJECT_ROOT is empty. Set project.root in ${MONAN_JEDI_CONFIG}."
    exit 1
  }
  [[ -n "${STACK_INSTANCE:-}" ]] || {
  ...
  [[ -n "${MONAN_JEDI_BUILD_ID:-}" ]] || {
    log_error "MONAN_JEDI_BUILD_ID is empty. Set build.id in ${MONAN_JEDI_CONFIG}."
    exit 1
  }
  ...
  # work/ is private/rebuildable; build/ is the published runtime.
  export MONAN_JEDI_WORK_ROOT="${MONAN_JEDI_WORK_ROOT:-${PROJECT_ROOT}/work/${MONAN_JEDI_BUILD_ID}}"
  export MONAN_JEDI_LOG_ROOT="${MONAN_JEDI_LOG_ROOT:-${PROJECT_ROOT}/logs/${MONAN_JEDI_BUILD_ID}}"
  ...
  export MONAN_JEDI_INSTALL_ROOT="${MONAN_JEDI_INSTALL_ROOT:-${PROJECT_ROOT}/build/${MONAN_JEDI_BUILD_ID}}"
